...
Page({
  ...
  havepass(e) {
    let url = app.globalData.url_online.url_9104 + 'room_service/company_update_sweep/' + that.data.MId + '/';
    http.postReq(url, { 
      remark: '打扫房间' + that.data.room_no + '成功'
    }, function (res) {
      wx.showToast({
        title: '打扫成功',
        icon: 'success'
      })
      console.log('打扫成功', res.data);
