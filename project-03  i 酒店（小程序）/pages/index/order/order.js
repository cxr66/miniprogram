...
Page({
   ...
   *  */
  reduce_guest() {
    if (that.data.guestList.length > 0) {
      that.data.guestList.pop();
      that.setData({
        guestList: that.data.guestList,
        guestHide: false
      })
  ...
  bindinput(e) {
    ...
    } else if (e.currentTarget.dataset.input_flag === 'price') {
      this.setData({
  ...
  onLoad: function (options) {
    ...
    http.postReq(app.globalData.url_online.url_9101 + 'rate_code/get_rate_code/', {
      ...
      // [that.data.room_type_code][that.data.on_date]
      if (res.data.price[0].price_list[0].room_price || res.data.price[0].price_list[0].room_price == 0) {
        that.setData({
  ...
  empty_face_to_room(room_number) {
    let url = 'https://equipments.eloadspider.com/v2/equipment/ht/cateye/empty_face_to_room/',
      params = {
  ...
  add_face_to_room(room_number, face_url, cb) {
    ...
    let url = 'https://equipments.eloadspider.com/v2/equipment/ht/cateye/add_face_to_room/',
      params = {
        'room_number': room_number,
        'face_url': face_url
      };
    ...
    return new Promise((resolve, reject) => {
      wx.request({
        ...
        data: {
          ...
          'signCode': hexMD5.hexMD5('76D00D59AB5F42AE99E0711B4F19081F,' + id_no + ',' + that.data.on_date + ' 12:00:00,7E435E10870D4C2FA42EC0979FC3C07B') // MD5加密：校验码(“keyID,cardID,reqTime,AppSecret”)
        },
  ...
  /** *
   * @asyncWithAwait 通过身份证取神盾图像，并且传到服务器，再添加faceId
   */
  async asyncWithAwait(id_no) {
    return new Promise((resolve, reject) => {
      /* 多人入住查看同住人的信息 */
      wx.request({
        url: app.globalData.url_online.url_public + 'SDSVCApi/YLShangHai/Get',
              ...
              wx.uploadFile({
                ...
                success(res) {
                  ...
                  if (data.message == 'success') {
                    let url = app.globalData.url_online.url_eq + 'equipment/ht/cateye/add_face_to_room/',
                      params = {
  ...
  order() {
    if (that.data.hotel_type == 0) {
      ...
      that.order_hotel_new();
    }
  },
  /**
   * @order_hotel_new 增加新的预定（任京接口）
   */
  order_hotel_new(e) {
