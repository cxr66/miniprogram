...
Page({
  ...
  onLoad: function(options) {
    ...
    http.getReq(app.globalData.url_online.url_9102 + 'ordering/master_base_list/?code_market=ZZJ&page_size=300', function(res) {
      console.log('查询自助机在住单列表', res.data);
      wx.hideLoading();
      that.setData({
        zzjform: res.data.results
      })
    });
  },
  ...
  clickMenu: function(e) {
    ...
    if (that.data.menuList[this.data.currentTab].name == '在住订单') {
      ...
      http.getReq(app.globalData.url_online.url_9102 + 'ordering/reserve_base_list/?code_market=ZZJ&page_size=300', function (res) {
        console.log('查询自助机预定单列表', res.data);
        wx.hideLoading();
        that.setData({
          zzjform: res.data.results
        })
